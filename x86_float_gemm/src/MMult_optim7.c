...
// TODO: The result has so big difference with the baseline that I suspect there are some error in it
    ...
    for (int mBlockStart = 0; mBlockStart < M; mBlockStart += BLOCKSIZE) {
        for (int nBlockStart = 0; nBlockStart < N; nBlockStart += BLOCKSIZE) {
            for (int kBlockStart = 0; kBlockStart < K; kBlockStart += BLOCKSIZE) {
                ...
                // MMult_optim4_4(&A(mBlockStart, kBlockStart), &B(kBlockStart, nBlockStart), &C(mBlockStart, nBlockStart), BLOCKSIZE, BLOCKSIZE, BLOCKSIZE, lda, ldb, ldc);
...
// This version has the same error with MMult_optim6_2 nested with MMult_optim7_1
        ...
        for (int kBlockStart = 0; kBlockStart < K; kBlockStart += BLOCKSIZE) {
          ...
          for (int i = mBlockStart; i < mBlockStart + BLOCKSIZE; i += 4) {
              for (int j = nBlockStart; j < nBlockStart + BLOCKSIZE; j += 4) {
                ...
                b_p_j0 = &(B(kBlockStart, j));
                //   b_p_j1 = &(B(0, j + 1));
                //   b_p_j2 = &(B(0, j + 2));
                //   b_p_j3 = &(B(0, j + 3));

                a_i0_p = &(A(i, kBlockStart));
                a_i1_p = &(A(i + 1, kBlockStart));
                a_i2_p = &(A(i + 2, kBlockStart));
                a_i3_p = &(A(i + 3, kBlockStart));
...
// floating-point rounding error problem is successfully avoided!!!
        ...
        for (int kBlockStart = 0; kBlockStart < K; kBlockStart += BLOCKSIZE) {
          ...
          for (int i = mBlockStart; i < mBlockStart + BLOCKSIZE; i += 4) {
              for (int j = nBlockStart; j < nBlockStart + BLOCKSIZE; j += 4) {
                ...
                // use last value to initilize, in order to avoid floating-point rounding error problem
