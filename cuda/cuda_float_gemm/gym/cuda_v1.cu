...
__global__ void Sgemm( 
    ...
    // 计算出block内总的thread数目（这里实际上就是blockDim.x 和 .y）
    ...
    // 在global->shared阶段，读取一行需要的thread数目
    //（因为这里写死，每一个thread读取4个元素）
    // 每一个thread读取元素的数目，要和其计算的元素数目区分开
    const int A_TILE_THREAD_PER_ROW = BLOCK_SIZE_K / 4;
    const int B_TILE_THREAD_PER_ROW = BLOCK_SIZE_N / 4;

    // row number and col number that needs to be loaded by this thread
    // 这样我就知道，当前thread负责读取哪一行元素
    const int A_TILE_ROW_START = tid / A_TILE_THREAD_PER_ROW;
    const int B_TILE_ROW_START = tid / B_TILE_THREAD_PER_ROW;
    
    // 对应我也知道，当前thread读取那一列元素
    const int A_TILE_COL = tid % A_TILE_THREAD_PER_ROW * 4; 
    const int B_TILE_COL = tid % B_TILE_THREAD_PER_ROW * 4;

    // row stride that thread uses to load multiple rows of a tile
    // 然后计算，所有thread读完后，需要下移多少行，继续load还没有load的元素
...
int main(int argc, char** argv) {
    ...
    for (int run = 0 ; run < nIter; run ++ ) {
        // THREAD_SIZE:  单个thread在一个维度上计算的元素数
