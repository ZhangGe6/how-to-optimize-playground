...
// no transpose of As based on 9_1
// because I found transpose on my machine slows down the speed
// However, no transpose dramatically slows down in this version...
template <
    ...
    > 
__global__ void gemm_optim10_1( 
    ...
    for (int thread_y = 0; thread_y < THREAD_SIZE_Y; ++thread_y) {
        frag_a[0][thread_y] = As[0][ty * THREAD_SIZE_Y + thread_y][0];
    }
    ...
    do{
        ...
        for(int j=0; j<BLOCK_SIZE_K-1; ++j){
            ...
            #pragma unroll
            for (int thread_y = 0; thread_y < THREAD_SIZE_Y; ++thread_y) {
                frag_a[(j+1)%2][thread_y] = As[load_stage_idx][ty * THREAD_SIZE_Y + thread_y][j+1];
            }
            // load B from shared memory to register
            #pragma unroll
            for (int thread_x = 0; thread_x < THREAD_SIZE_X; thread_x += 4) {
        ...
        #pragma unroll
        for (int thread_y = 0; thread_y < THREAD_SIZE_Y; ++thread_y) {
            frag_a[0][thread_y] = As[load_stage_idx^1][ty * THREAD_SIZE_Y + thread_y][0];
        }
        // load B from shared memory to register
        #pragma unroll
        for (int thread_x = 0; thread_x < THREAD_SIZE_X; thread_x += 4) {
...
void MMult_optim10_1(cublasHandle_t handle, float *A, float *B, float *C, const int M, const int K, const int N, float alpha, float beta) {
    ...
    gemm_optim10_1<BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, THREAD_SIZE_Y, THREAD_SIZE_X, ENABLE_DOUBLE_BUFFER> <<<dimGrid, dimBlock>>>(A, B, C, M, K, N, alpha, beta);
