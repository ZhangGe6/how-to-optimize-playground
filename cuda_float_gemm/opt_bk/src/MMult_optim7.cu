...
__global__ void gemm_optim7_3( 
    ...
    __shared__ float As[BLOCK_SIZE_K][BLOCK_SIZE_M]; // avoid bank conflict
    __shared__ float Bs[BLOCK_SIZE_K][BLOCK_SIZE_N];
    // registers for C
    float accum[THREAD_SIZE_Y][THREAD_SIZE_X] = {0};
    // registers for A and B
    ...
    for (int tile_idx = 0 ; tile_idx < K ; tile_idx += BLOCK_SIZE_K) {
        ...
        #pragma unroll
        for (int i = 0 ; i < BLOCK_SIZE_M ; i += A_TILE_ROW_STRIDE) {
            // if As is not transposed, we have
                // As.shape = [BLOCK_SIZE_M][BLOCK_SIZE_K]
                // As[A_TILE_ROW_START + i][A_TILE_COL] = A[OFFSET(BLOCK_SIZE_M * by + A_TILE_ROW_START + i, A_TILE_COL + tile_idx, K)];
                // As[A_TILE_ROW_START + i][A_TILE_COL + 1] = A[OFFSET(BLOCK_SIZE_M * by + A_TILE_ROW_START + i, A_TILE_COL + tile_idx + 1, K)];
                // As[A_TILE_ROW_START + i][A_TILE_COL + 2] = A[OFFSET(BLOCK_SIZE_M * by + A_TILE_ROW_START + i, A_TILE_COL + tile_idx + 2, K)];
                // As[A_TILE_ROW_START + i][A_TILE_COL + 3] = A[OFFSET(BLOCK_SIZE_M * by + A_TILE_ROW_START + i, A_TILE_COL + tile_idx + 3, K)];
            // So if A is transposed, we have 
            // As.shape = [BLOCK_SIZE_K][BLOCK_SIZE_M]

            // way1:  slow
            // As[A_TILE_COL][A_TILE_ROW_START + i] = A[OFFSET(BLOCK_SIZE_M * by + A_TILE_ROW_START + i, A_TILE_COL + tile_idx, K)];
            // As[A_TILE_COL + 1][A_TILE_ROW_START + i] = A[OFFSET(BLOCK_SIZE_M * by + A_TILE_ROW_START + i, A_TILE_COL + tile_idx + 1, K)];
            // As[A_TILE_COL + 2][A_TILE_ROW_START + i] = A[OFFSET(BLOCK_SIZE_M * by + A_TILE_ROW_START + i, A_TILE_COL + tile_idx + 2, K)];
            // As[A_TILE_COL + 3][A_TILE_ROW_START + i] = A[OFFSET(BLOCK_SIZE_M * by + A_TILE_ROW_START + i, A_TILE_COL + tile_idx + 3, K)];
        ...
        #pragma unroll
        for (int k = 0; k < BLOCK_SIZE_K; ++ k) {
            // If A is transposed, we can use float4 datatype when loading A from shared memo into register
            // load A from shared memory to register
            // #pragma unroll
            // for (int thread_y = 0; thread_y < THREAD_SIZE_Y; ++thread_y) {
            //     frag_a[thread_y] = As[ty * THREAD_SIZE_Y + thread_y][k];
            ...
            // Note: THREAD_SIZE_Y % 4 == 0 should be garanteened if we use float4 here
            #pragma unroll
            for (int thread_y = 0; thread_y < THREAD_SIZE_Y; thread_y += 4) {
                // frag_a[thread_y] = As[ty * THREAD_SIZE_Y + thread_y][k];
                FETCH_FLOAT4(frag_a[thread_y]) = FETCH_FLOAT4(As[k][ty * THREAD_SIZE_Y + thread_y]);
            }
...
void MMult_optim7_3(cublasHandle_t handle, float *A, float *B, float *C, const int M, const int K, const int N, float alpha, float beta) {
    ...
    gemm_optim7_3<BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, THREAD_SIZE_Y, THREAD_SIZE_X, ENABLE_DOUBLE_BUFFER> <<<dimGrid, dimBlock>>>(A, B, C, M, K, N, alpha, beta);
    gpuErrchk( cudaPeekAtLastError() );
    gpuErrchk( cudaDeviceSynchronize() );
}


// only use float4 for Bs
template <
    const int BLOCK_SIZE_M,  // width of block of C that each thread block calculate
    const int BLOCK_SIZE_K,  // height of block of A that each thread block load into shared memory
    const int BLOCK_SIZE_N,  // height of block of C that each thread block calculate
...
void MMult_optim7_4(cublasHandle_t handle, float *A, float *B, float *C, const int M, const int K, const int N, float alpha, float beta) {
    ...
    gemm_optim7_4<BLOCK_SIZE_M, BLOCK_SIZE_K, BLOCK_SIZE_N, THREAD_SIZE_Y, THREAD_SIZE_X, ENABLE_DOUBLE_BUFFER> <<<dimGrid, dimBlock>>>(A, B, C, M, K, N, alpha, beta);
    gpuErrchk( cudaPeekAtLastError() );
    gpuErrchk( cudaDeviceSynchronize() );
}
